...
const double tnum=4*2*pow(10,9)*zoombeta;	//number of time step
...
void outputenergy(double j,double energy,double peroid)
{
	if(ceil(j/peroid)==(j/peroid))
	{
		ofstream phifile;
...
int main(int argc, char *argv[])
{
	...
	for(int j=0;j<=tnum;j++)				//time step calculating
	{
		...
		outputenergy(j,energycal(r,dr,phi,pdtphi,pdrphi),peroid);		//output energy
//-------------------Euler step
/*		#pragma omp parallel
		{
			#pragma omp for
